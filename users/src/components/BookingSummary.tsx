...
const BookingSummary: React.FC<BookingSummaryProps> = ({
  ...
  return (
    <Card className="border-none shadow-[0_10px_40px_-15px_rgba(0,0,0,0.1)] rounded-3xl sticky top-24 overflow-hidden">
      ...
      <CardContent className="space-y-6 pt-6">
        <div className="space-y-4">
            ...
            <p className="text-sm font-medium text-slate-500">Facility</p>
            <p className="font-bold text-slate-800 text-right">{facilityName}</p>
            </div>
            
            <div className="flex justify-between items-center pb-3 border-b border-slate-100 border-dashed">
            <p className="text-sm font-medium text-slate-500">Pitch</p>
            <p className="font-bold text-slate-800">{pitchName}</p>
            </div>
            ...
            <div className="flex justify-between items-center pb-3 border-b border-slate-100 border-dashed">
                ...
                <p className="font-bold text-slate-800 bg-emerald-50 text-emerald-700 px-3 py-1 rounded-lg">
                    {`${selectedTimeSlot.startTime} - ${selectedTimeSlot.endTime}`}
                </p>
            </div>
            ) : (
            <div className="flex justify-between items-center pb-3 border-b border-slate-100 border-dashed text-slate-400">
            ...
            <p className="text-base font-bold text-slate-800">Total Price</p>
            <p className="text-2xl text-emerald-600 font-extrabold flex items-center gap-1">
                <span className="text-sm text-emerald-500">NPR</span> {pricePerHour}
            </p>
            </div>
        </div>
        ...
        <div className="pt-4">
            <p className="text-sm font-bold text-slate-800 mb-3">Select Payment Method</p>
            <div className="grid grid-cols-1 gap-3">
                
                <label className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${gateway === 'cash' ? 'border-emerald-500 bg-emerald-50/50' : 'border-slate-100 hover:border-emerald-200'}`}>
                    <input type="radio" className="hidden" name="gateway" value="cash" checked={gateway === 'cash'} onChange={() => setGateway('cash')} />
                    <div className={`h-4 w-4 rounded-full border flex items-center justify-center ${gateway === 'cash' ? 'border-emerald-500' : 'border-slate-300'}`}>
                        {gateway === 'cash' && <div className="h-2 w-2 rounded-full bg-emerald-500" />}
                    </div>
                    <Banknote className={`h-5 w-5 ${gateway === 'cash' ? 'text-emerald-600' : 'text-slate-400'}`} />
                    <span className={`font-semibold ${gateway === 'cash' ? 'text-emerald-800' : 'text-slate-600'}`}>Pay with Cash on site</span>
                </label>
                
                <label className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${gateway === 'esewa' ? 'border-[#60b545] bg-[#60b545]/5' : 'border-slate-100 hover:border-[#60b545]/30'}`}>
                    <input type="radio" className="hidden" name="gateway" value="esewa" checked={gateway === 'esewa'} onChange={() => setGateway('esewa')} />
                    <div className={`h-4 w-4 rounded-full border flex items-center justify-center ${gateway === 'esewa' ? 'border-[#60b545]' : 'border-slate-300'}`}>
                ...
                <label className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${gateway === 'khalti' ? 'border-[#5C2D91] bg-[#5C2D91]/5' : 'border-slate-100 hover:border-[#5C2D91]/30'}`}>
                    <input type="radio" className="hidden" name="gateway" value="khalti" checked={gateway === 'khalti'} onChange={() => setGateway('khalti')} />
                    <div className={`h-4 w-4 rounded-full border flex items-center justify-center ${gateway === 'khalti' ? 'border-[#5C2D91]' : 'border-slate-300'}`}>
      ...
      <CardFooter className="bg-slate-50/50 pt-4 pb-6 px-6 border-t border-slate-100 flex flex-col gap-3">
        <Button
          onClick={handleBooking}
          disabled={!selectedTimeSlotId || !isAuthenticated || isProcessing}
          className={`w-full h-14 rounded-xl font-bold text-[16px] shadow-lg transition-transform ${gateway === 'cash' ? 'bg-emerald-600 hover:bg-emerald-700 hover:-translate-y-1 shadow-emerald-600/20' : gateway === 'esewa' ? 'bg-[#60b545] hover:bg-[#4d9735] hover:-translate-y-1 shadow-[#60b545]/20' : 'bg-[#5C2D91] hover:bg-[#4a2474] hover:-translate-y-1 shadow-[#5C2D91]/20'}`}
        >
          {isProcessing ? (
        ...
        <p className="text-center w-full text-xs text-slate-400 font-medium leading-relaxed">By clicking confirm, you agree to the facility's cancellation rules and terms of service.</p>
      </CardFooter>
    </Card>
  );
};
